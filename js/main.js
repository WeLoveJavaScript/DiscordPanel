...
Object.keys(locales["de"]).forEach((key) => {
    if (typeof locales["de"][key] === "string") {
        if (localeFile[key] === ("" || undefined)) {
            localeFile[key] = locales["de"][key];
        }
    } else if (typeof locales["de"][key] === "object") {
        if (!localeFile[key]) {
            localeFile[key] = locales["de"][key];
        }
    } else {
        Object.keys(locales["de"][key]).forEach((subKey) => {
            if (localeFile[key][subKey] === ("" || undefined)) {
                localeFile[key][subKey] = locales["de"][key][subKey];
            }
        });
    }
